...
class AbstractMaterializedSqlViewTester(SharedSetupTransactionCase):
    ...
    def test_upgrade_db90300(self):
        ...
        self.demo_matview_mdl.create_materialized_view(cr, SUPERUSER_ID, context=ctxt)
        # Make sure mat view relation is a table
        cr.execute("SELECT relkind FROM pg_class WHERE relname = '%(relname)s'" %
        ...
        self.demo_matview_mdl.create_or_upgrade_pg_matview_if_needs(cr, SUPERUSER_ID, context=ctxt)
        cr.execute("SELECT relkind FROM pg_class WHERE relname = '%(relname)s'" %
                   {'relname': self.demo_matview_mdl._sql_mat_view_name})
        ...
        self.demo_matview_mdl.drop_materialized_view_if_exist(cr, SUPERUSER_ID, 90300, context=ctxt)
        ctxt.update({'force_pg_version': 90200})
        self.demo_matview_mdl.create_materialized_view(cr, SUPERUSER_ID, context=ctxt)
        cr.execute("SELECT relkind FROM pg_class WHERE relname = '%(relname)s'" %
                   {'relname': self.demo_matview_mdl._sql_mat_view_name})
        self.assertEquals(cr.fetchone()[0], 'r')
        ctxt.update({'force_pg_version': 90300})
        self.demo_matview_mdl.refresh_materialized_view(cr, SUPERUSER_ID, context=ctxt)
        cr.execute("SELECT relkind FROM pg_class WHERE relname = '%(relname)s'" %
                   {'relname': self.demo_matview_mdl._sql_mat_view_name})
        self.assertEquals(cr.fetchone()[0], 'm')
    ...
    def test_upgrade_mat_view(self):
        ...
        self.mat_view_mdl.write_values(cr, uid, self.demo_matview_mdl._sql_mat_view_name,
        ...
        self.assertEquals([],
                          self.demo_matview_mdl.create_or_upgrade_pg_matview_if_needs(
                              cr, SUPERUSER_ID, context=ctxt))
        cr.execute("ALTER VIEW %s RENAME TO test" % self.demo_matview_mdl._sql_view_name)
        self.mat_view_mdl.write_values(cr, uid, self.demo_matview_mdl._sql_mat_view_name,
                                       {'view_name': 'test'},
                                       context=ctxt)
        self.assertNotEquals([],
