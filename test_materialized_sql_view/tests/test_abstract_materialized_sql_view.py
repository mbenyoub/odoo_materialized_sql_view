...
class AbstractMaterializedSqlViewTester(SharedSetupTransactionCase):
    ...
    def test_read_and_refresh_materialized_view(self):
        ...
        user_count = self.demo_mdl.read(cr, uid, self.group_id, ['user_count'],
        ...
        self.assertEquals(self.demo_mdl.read(cr, uid, self.group_id, ['user_count'],
                                             context=self.context)['user_count'],
                          user_count)
        # Refresh the materialized view
        self.demo_mdl.refresh_materialized_view(cr, SUPERUSER_ID, context=self.context)
